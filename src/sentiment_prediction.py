...
def read_data(input_data: str) -> pd.DataFrame:
    """
...
def perform_sentiment_analysis(
    ...
    print(
        f"[{model_name}] Inference Time: {inference_time:.2f} seconds for {len(reviews)} reviews"
    )
...
def generate_output(
    ...
    sentiments: List[str],
    model_name: str,
    filename: str = "sentiment_output.csv",
) -> None:
    df = pd.DataFrame({"review": reviews, "sentiment": sentiments, "model": model_name})
...
def benchmark(
    ...
    for model_name, sentiments in zip(models, sentiments_list):
        ...
        percentage_of_population = np.arange(1, len(labels_sorted) + 1) / len(
            labels_sorted
        )
